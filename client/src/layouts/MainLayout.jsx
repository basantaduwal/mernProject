        ...
        <nav className="flex items-center h-16 gap-4">
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-2xl mx-auto">
            <div className="relative">
              <input
                type="text"
                className="input-field pl-4 pr-10 h-10 text-sm"
              ...
              <button
                type="submit"
                className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md text-orange-400 hover:text-orange-300 transition-colors"
              >
            ...
            {isAuthenticated ? (
              <div className="relative">
                ...
                >
                  ...
                  <span className="text-white/80 hidden md:block max-w-[100px] truncate">{user?.name}</span>
                  <svg className={`w-4 h-4 text-white/50 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 glass rounded-xl border border-white/10 shadow-2xl py-1 z-50">
                    ...
                    {isAdmin && (
                      <Link to="/admin" onClick={() => setDropdownOpen(false)}
                        className="flex items-center gap-2 px-4 py-2 text-sm text-orange-400 hover:text-orange-300 hover:bg-white/5 transition-all">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="border-t border-white/10 mt-1">
                      <button onClick={handleLogout}
                        className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-white/5 transition-all">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const Footer = () => (
  <footer className="glass-dark border-t border-white/[0.06] mt-auto">
    <div className="page-container py-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div>
          <h4 className="text-sm font-semibold text-white/70 mb-3 uppercase tracking-wider">Shop</h4>
          <ul className="space-y-2">
            {['All Products', 'Electronics', 'Clothing', 'Footwear'].map((item) => (
              <li key={item}>
                <Link to="/products" className="text-sm text-white/40 hover:text-orange-400 transition-colors">{item}</Link>
              </li>
            ))}
          </ul>
        </div>
        <div>
          <h4 className="text-sm font-semibold text-white/70 mb-3 uppercase tracking-wider">Account</h4>
          <ul className="space-y-2">
            {[['Profile', '/profile'], ['My Orders', '/orders'], ['Cart', '/cart']].map(([label, path]) => (
              <li key={label}>
                <Link to={path} className="text-sm text-white/40 hover:text-orange-400 transition-colors">{label}</Link>
              </li>
            ))}
          </ul>
        </div>
